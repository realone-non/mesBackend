...
//            description = "검색조건: 품목그룹 id, 품명|품번, 수주번호, 제조오더번호, 작업공정 id, 착수예정일 fromDate~endDate, 지시상태"
//    )
...
//    ) {
//        List<ProduceOrderResponse> produceOrders = workOrderService.getProduceOrders(itemGroupId, itemNoAndName, contractNo, produceOrderNo, workProcessId, fromDate, toDate, instructionStatus);
...
//    @PostMapping("/{produce-order-id}/work-orders")
//    @ApiResponses(
//            value = {
...
//
//    // 작업지시 단일조회
//    @GetMapping("/{produce-order-id}/work-orders/{work-order-id}")
//    @ResponseBody()
...
//            }
//    )
//    public ResponseEntity<ProduceOrderResponse> getWorkOrder(
//            @PathVariable(value = "produce-order-id") @Parameter(description = "제조오더 id") Long produceOrderId,
//            @PathVariable(value = "work-order-id") @Parameter(description = "작업지시 id") Long workOrderId,
//            @RequestHeader(value = "Authorization", required = false) @Parameter(hidden = true) String tokenHeader
//    ) throws NotFoundException {
//        ProduceOrderResponse produceOrder = workOrderService.getWorkOrder(produceOrderId);
//        cLogger = new MongoLogger(logger, "mongoTemplate");
//        cLogger.info(logService.getUserCodeFromHeader(tokenHeader) + " is viewed the " + produceOrder.getId() + " from getWorkOrder.");
//        return new ResponseEntity<>(produceOrder, HttpStatus.OK);
//    }
//
//    // 작업지시 리스트 조회
//    @GetMapping("{produce-order-id}/work-orders")
//    @ResponseBody
//    @Operation(summary = "작업지시 리스트 조회", description = "")
//    public ResponseEntity<List<ProduceOrderResponse>> getWorkOrders(
//            @PathVariable(value = "produce-order-id") @Parameter(description = "제조오더 id") Long produceOrderId,
//            @RequestHeader(value = "Authorization", required = false) @Parameter(hidden = true) String tokenHeader
//    ) {
//        List<ProduceOrderResponse> produceOrders = workOrderService.getProduceOrders(produceOrderId);
//        cLogger = new MongoLogger(logger, "mongoTemplate");
//        cLogger.info(logService.getUserCodeFromHeader(tokenHeader) + " is viewed the list of from getProduceOrders.");
//        return new ResponseEntity<>(produceOrders, HttpStatus.OK);
//    }
