...
public class ShipmentItem extends BaseTimeEntity {
    ...
    @ManyToOne(fetch = LAZY)
    @JoinColumn(name = "SHIPMENT", columnDefinition = "bigint COMMENT '출하'", nullable = false)
...
//    private float gw;                 // G/W(KG)
    ...
    public void create(Shipment shipment, ContractItem contractItem, String note) {
        setShipment(shipment);
        setContractItem(contractItem);
        setNote(note);
    }

    public void update(ContractItem newContractItem, String newNote) {
